...
public class CPlayRoomUI : MonoBehaviour, IMessageReceiver {
	...
	public Transform floor_slot_root;
	public List<Vector3> floor_slot_position;


	public Transform deck_slot;


	public CPlayerCardPosition[] player_card_positions;
	...
	void make_deck_cards()
	{
		...
		for (int i = 0; i < this.total_card_pictures.Count; ++i)
		{
			...
			this.total_card_pictures[i].sprite_renderer.sortingOrder =
				CSpriteLayerOrderManager.Instance.Order; //화면상의 겹치는 구간 제거
		}
	}
	...
	Sprite get_hwatoo_sprite(CCard card)
	{
		...
		return this.hwatoo_sprites[sprite_index];
	}
